  ...
  return (
    <div className="bg-base-300 flex justify-between p-3 rounded-2xl lg:px-20">
      ...
      <div className="flex justify-end items-center gap-4">
        ...
        {user ? (
          <button className="bg-primary px-3 py-2 rounded-xl font-semibold text-sm" onClick={logout}>Logout</button>
        ) : (
          <Link className="bg-primary px-3 py-2 rounded-xl font-semibold text-sm" to="/login">Login</Link>
        )}
      </div>
    </div>
  );
